...
- M2 legacy continuation benchmark report/gate CLI (`ls.m2.legacy_benchmark_report@1.0.0`) with RFC-002 artifact envelope output, median/p90 efficiency metrics, quality-floor invalid-gain semantics, checked-in threshold config and sample report artifact, CI enforcement, and benchmark CLI regression coverage (`benchmarks/run-harness.mjs`, `benchmarks/tasks.json`, `benchmarks/legacy-benchmark-gates-thresholds.v1.json`, `benchmarks/reports/legacy-benchmark-report.json`, `benchmarks/README.md`, `.github/workflows/ci.yml`, `runtime/test/benchmark-harness.test.ts`, `#55`).
- Runtime apply/rollback control artifact (`ls.m2.apply_rollback_record@1.0.0`) with strict PR-bundle validation, policy/capability/approval gating, rollback-availability and target-state precondition checks, optional benchmark-valid-gain enforcement, deterministic local apply/rollback execution snapshots, and runtime reliability coverage (`runtime/src/apply-rollback.ts`, `runtime/src/index.ts`, `runtime/test/apply-rollback.test.ts`, `runtime/README.md`, `#56`).
